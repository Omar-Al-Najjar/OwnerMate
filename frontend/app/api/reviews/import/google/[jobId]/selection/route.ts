  ...
  try {
    ...
    if (!payload.candidateId?.trim()) {
      throw new BackendRouteError(
        "GOOGLE_IMPORT_CANDIDATE_REQUIRED",
        "Choose a Google place before continuing the import.",
        422
      );
    }
    ...
    const result = await fetchBackendJson<GoogleImportJobResponse>(
      `/reviews/import/google/${jobId}/selection`,
      {
        method: "POST",
        headers,
        body: JSON.stringify({
          candidate_id: payload.candidateId.trim(),
        }),
      }
    );
    ...
    return NextResponse.json(
      {
        ...
        error: {
          ...
          message: "Google place selection failed.",
        },
      },
      { status: 500 }
    );
